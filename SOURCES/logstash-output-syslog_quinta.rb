...
#    sourcehost = event.sprintf(@sourcehost)
    ...
    facility_code = FACILITY_LABELS.index(event["facility"]) || FACILITY_LABELS.index(@facility)

    severity_code = event["level"] || SEVERITY_LABELS.index(@severity)

    priority = (facility_code * 8) + severity_code

    if rfc3164?
      timestamp = event.sprintf("%{+MMM dd HH:mm:ss}")
      syslog_msg = "<"+priority.to_s()+">"+event["timestamp"]+" "+event["source_host"]+" "+event["appname"]+"["+event[procid]+"]: "+event["message"]
    else
      syslog_msg = "<"+priority.to_s()
      syslog_msg += ">1 "+(event["@timestamp"].to_datetime.iso8601(6) || "-") #strftime("%Y-%m-%dT%H:%M:%S.SSSZ}")
      syslog_msg += " "+(event["host"] || "-")
      syslog_msg += " "+(event["SYSLOG_IDENTIFIER"] || "-")
      syslog_msg += " "+(event["_PID"] || "-")
      syslog_msg += " "+(event["msgid"] || "-")
      syslog_msg += " - " + (event["message"] || "-")
#      syslog_msg = "<"+priority.to_s()+">1 "+event["timestamp"]+" "+event["source_host"]+" "+event["appname"]+" "+event["procid"]+" "+event["msgid"]+" - "+event["message"]
    end
